...
void fillRandom(vector<vector<int>> &matrix) {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            matrix[i][j] = rand() % (b - a + 1) + a; //-100 до 100 201 - 100
        }
    }
...
void Darray17(vector<vector<int>> &matrix) {
    int m, count1 = 1, count2 = 1, count = 1; //единичная, диагональная, нулеваяю предположим что матрицы сразу же все три типа при проверки если находим не правильные эл-ты то эти переменные обнуляемы
    int n = matrix.size(); //размер строк
    m = n;
    m = matrix[0].size();
    for (int i = 0; i < n; i++) {
